...
| `src/cleaners.py` | Filters physically-impossible value spikes (DAS glitches), nighttime/offline rows (any inverter at zero), inverter CT/communication dropouts (one inverter's share of generation collapsing vs its baseline), per-station meter CT/communication dropouts (one generation-meter station's share of total meter output collapsing vs its baseline, for multi-meter sites), single-phase CT faults (one meter station's phase current — `IacA`/`IacB`/`IacC` — deviating from the per-row median of its three legs by more than `config.MAX_PHASE_CURRENT_DEVIATION`, which catches a dropped phase that is too small a fraction of station energy to trip the share check), and gross outliers (efficiency outside the sensor-sanity band). The comms filters apply a hard absolute rule first: any individual channel (inverter, meter station, or phase leg) reading **negative while the site is producing** is dropped as a physical impossibility *before* the statistical share/median baseline is computed — so a run of bad intervals cannot corrupt the baseline and blind the deviation check. |
